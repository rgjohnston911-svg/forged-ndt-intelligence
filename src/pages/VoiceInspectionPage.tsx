// DEPLOY472 -- src/pages/VoiceInspectionPage.tsx v16.24 (Tier 1B quarantine: 152 orphan engine files moved to archive/ [no longer deployed as public /api]; system-check.html removed from deploy; LIVE-ENGINES manifest + CI reachability/forward-key check)
// DEPLOY471 -- src/pages/VoiceInspectionPage.tsx v16.24 (Tier 1B guards: 28 spend/data endpoints auth-guarded [combined JWT-or-server-key]; server-key forwarding on all 10 internal callers; frontend JWT wired on case-search/escalation/CaseDetail run+authority; core decision path left open)
// DEPLOY470 -- src/pages/VoiceInspectionPage.tsx v16.24 (Tier 2 correctness: unmanned/unoccupied no longer fabricates FATAL human-occupancy [word-boundary + NUI downgrade]; FMD interaction gate consumes FMD's own observed per-family states so genuine CUI+fatigue/MIC+HIC/erosion+SCC interactions are restored while the TEST 36 phantom stays suppressed)
// DEPLOY468 -- src/pages/VoiceInspectionPage.tsx v16.24 (TEST 36 close-out: CP3 satellites consume the one verdict - no fabricated MIC/HIC synergy, no phantom crack-NDE; authority follows the governing reality - assurance -> IEC 61511/ISA 18.2/PSM, physical codes -> reference; recognizer states the protection-layer answers; dynamic-loading leveled beneath the control root -> restricted)
...
function generateInspectionReport(data: {
  ...
  html += "<div class='subtitle'>v16.24 | Engine: " + (dc.engine_version || "decision-core") + " + Authority Lock v1.0 + Remaining Strength v1.1 + FMD v1.3.2 + Disposition Pathway v1.1 + Failure Timeline v1.1 + Photo Analysis v1.4 + Superbrain v1.2 + Provenance v1.0 | Elapsed: " + (dc.elapsed_ms || "?") + "ms</div>";
  html += "</div>";
  ...
  html += "<div style='font-size:14px;font-weight:900;color:#000;margin-bottom:8px;'>HARDENING DIAGNOSTIC (v16.24 / DEPLOY468 TEST 36 close-out)</div>";
  ...
  html += "Generated by FORGED NDT Intelligence OS v16.24 - " + esc(dateStr) + " " + esc(timeStr) + " - " + esc(caseRef);
  html += "</div>";
  ...
  return (
    <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "20px" }}>
      <div style={{ marginBottom: "24px" }}>
        <h1 style={{ fontSize: "22px", fontWeight: 800, margin: "0 0 4px 0", color: "#111" }}>FORGED NDT Intelligence OS {"\u2014"} v16.24</h1>
      ...
      <div ref={resultsRef}>
        ...
        {(authorityLockResult || remainingStrengthResult || failureModeDominanceResult || dispositionPathwayResult || failureTimelineResult) && (
          <div style={{ marginBottom: "16px", padding: "12px", border: "2px solid #000", borderRadius: "8px", backgroundColor: "#fffbe6" }}>
            <div style={{ fontSize: "14px", fontWeight: 800, marginBottom: "8px" }}>Build 1+2+3 Engine Results (v16.24 inline diagnostic)</div>
