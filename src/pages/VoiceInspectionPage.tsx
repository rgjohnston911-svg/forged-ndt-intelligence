// DEPLOY473 -- src/pages/VoiceInspectionPage.tsx v16.25 (hotfix: comprehensive-assessment + differential-diagnosis auth guard was trapped inside the OPTIONS block and never ran for POST -> moved after it; both now 401 anonymous)
// DEPLOY472 -- src/pages/VoiceInspectionPage.tsx v16.25 (Tier 1B quarantine: 152 orphan engine files moved to archive/ [no longer deployed as public /api]; system-check.html removed from deploy; LIVE-ENGINES manifest + CI reachability/forward-key check)
// DEPLOY471 -- src/pages/VoiceInspectionPage.tsx v16.25 (Tier 1B guards: 28 spend/data endpoints auth-guarded [combined JWT-or-server-key]; server-key forwarding on all 10 internal callers; frontend JWT wired on case-search/escalation/CaseDetail run+authority; core decision path left open)
// DEPLOY470 -- src/pages/VoiceInspectionPage.tsx v16.25 (Tier 2 correctness: unmanned/unoccupied no longer fabricates FATAL human-occupancy [word-boundary + NUI downgrade]; FMD interaction gate consumes FMD's own observed per-family states so genuine CUI+fatigue/MIC+HIC/erosion+SCC interactions are restored while the TEST 36 phantom stays suppressed)
// DEPLOY468 -- src/pages/VoiceInspectionPage.tsx v16.25 (TEST 36 close-out: CP3 satellites consume the one verdict - no fabricated MIC/HIC synergy, no phantom crack-NDE; authority follows the governing reality - assurance -> IEC 61511/ISA 18.2/PSM, physical codes -> reference; recognizer states the protection-layer answers; dynamic-loading leveled beneath the control root -> restricted)
// DEPLOY465 -- src/pages/VoiceInspectionPage.tsx v16.19 (generalized assurance recognizer: one behavior-keyed recognizer covers SIS/ESD AND a control loop on a vessel; no-assurance-default; SAFETY->IEC 61511 / CONTROL->ISA 18.2 routing)
// DEPLOY464 -- src/pages/VoiceInspectionPage.tsx v16.18 (functional_safety asset class: SIS/ESD declared-identity -> IEC 61511 LOCKED -> passes identity gate -> assurance recognizer concludes)
// DEPLOY463 -- src/pages/VoiceInspectionPage.tsx v16.17 (Asset Identity Gate: decision-core HOLDs when authority-lock cannot map a governing authority - ESD->bridge_concrete becomes a clean HOLD, not a fabricated cascade)
// DEPLOY462 -- src/pages/VoiceInspectionPage.tsx v16.16 (CP4a: safety-function assurance recognizer - SIS/ESD demand-vs-response divergence or logic change w/o independent validation -> assurance UNKNOWN_STATE -> verify the safety function)
// DEPLOY461 -- src/pages/VoiceInspectionPage.tsx v16.15 (CP3 commit 5: consequence failure_mode gated on evidence - no manufactured crack_propagation_pressure_breach on a clean asset)
// DEPLOY460 -- src/pages/VoiceInspectionPage.tsx v16.14 (CP3 commit 4: disposition ledger + inspection plan gate on evidence - no full ledger/NDE for a phantom mechanism)
// DEPLOY459 -- src/pages/VoiceInspectionPage.tsx v16.13 (CP3 commit 3: FMD corrosion/cracking/structural sub-paths gate on the evidence verdict - NONE -> inactive, no severity)
// DEPLOY458 -- src/pages/VoiceInspectionPage.tsx v16.12 (CP3 commit 2: Authority Lock mechanism-triggered locks gate on the evidence verdict; asset-class locks unchanged)
// DEPLOY456 -- src/pages/VoiceInspectionPage.tsx v16.11 (governance contest CP2: runGovernanceContest is the sole disposition arbiter; reconcile governing-reality reflects merge-not-escalate)
...
function generateInspectionReport(data: {
  ...
  html += "<div class='subtitle'>v16.25 | Engine: " + (dc.engine_version || "decision-core") + " + Authority Lock v1.0 + Remaining Strength v1.1 + FMD v1.3.2 + Disposition Pathway v1.1 + Failure Timeline v1.1 + Photo Analysis v1.4 + Superbrain v1.2 + Provenance v1.0 | Elapsed: " + (dc.elapsed_ms || "?") + "ms</div>";
  html += "</div>";
  ...
  html += "<div style='font-size:14px;font-weight:900;color:#000;margin-bottom:8px;'>HARDENING DIAGNOSTIC (v16.25 / DEPLOY468 TEST 36 close-out)</div>";
  ...
  if (fmd) {
    ...
    // DEPLOY452 - mechanism-compulsion gate. When FMD reports NONE, no confirmed physical
    // damage mechanism is established. Render an explicit deferral-to-Governing-Reality line
    // instead of a contradictory bare "GOVERNING MECHANISM: NONE" - the system is allowed to
    // conclude there is no governing damage mechanism.
    if (fmd.governing_failure_mode === "NONE" && !hasSuspectedGov) {
      html += "<div class='banner' style='background:#475569'>NO CONFIRMED DAMAGE MECHANISM ESTABLISHED \u2014 the account contains no direct, non-negated evidence of corrosion, cracking, or structural instability. Disposition is set by the Governing Reality (regulatory / assurance / operational), not by a physical mechanism.</div>";
    } else {
  ...
  html += "Generated by FORGED NDT Intelligence OS v16.25 - " + esc(dateStr) + " " + esc(timeStr) + " - " + esc(caseRef);
  html += "</div>";
  ...
  return (
    <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "20px" }}>
      <div style={{ marginBottom: "24px" }}>
        <h1 style={{ fontSize: "22px", fontWeight: 800, margin: "0 0 4px 0", color: "#111" }}>FORGED NDT Intelligence OS {"\u2014"} v16.25</h1>
      ...
      <div ref={resultsRef}>
        ...
        {(authorityLockResult || remainingStrengthResult || failureModeDominanceResult || dispositionPathwayResult || failureTimelineResult) && (
          <div style={{ marginBottom: "16px", padding: "12px", border: "2px solid #000", borderRadius: "8px", backgroundColor: "#fffbe6" }}>
            <div style={{ fontSize: "14px", fontWeight: 800, marginBottom: "8px" }}>Build 1+2+3 Engine Results (v16.25 inline diagnostic)</div>
